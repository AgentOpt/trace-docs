...
Trace supports multiple optimization algorithms. Choose the one that fits your needs.
...
<Cards>
  ...
  <Card 
    ...
    description="Trace's flagship optimizer (recommended)"
  />
</Cards>
...
|-----------|-------|-------------|----------|
| **OptoPrime** ⭐ | ⚡⚡ Fast | Full graph | Most use cases |
| **OPRO** | ⚡⚡⚡ Very Fast | None | Simple prompts |
| **TextGrad** | 🐌 Slower | Partial graph | Large graphs |

## Switching is Easy

One of Trace's key features is that switching optimizers requires changing just one line:
...
All optimizers in Trace follow the same basic pattern:
